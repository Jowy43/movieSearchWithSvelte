<script>
  ...
  $: if (value.length > 2) {
    loading = true
    fetch(`http://www.omdbapi.com/?apikey=11a5eaf1&s=${value}`)
      .then((res) => res.json())
      .then((apiResponse) => {
...
<div class="search-input">
  <input class="input-text" type="text" on:input={handleInput} placeholder="Search movies... " />
  {#if loading}
